...
var musica5 = new Musica { Titulo = "Geração Coca-Cola", Artista = "Legião Urbana", Duracao = 312 };
...
//ObterMusicaPeloTitulo("Eduardo e Mônica");

//ObterMusicaAleatoria();
...
{
    private Queue<Musica> fila = new Queue<Musica>(); // enfileirar, primeiro a entrar, primeiro a sair (FIFO)
...
class Playlist : ICollection<Musica>
{
    ...
    public Musica? ObterMusicaAleatoria()
    {
        if (lista.Count > 0)
        {
            ...
            var numAleatorio = random.Next(0, lista.Count - 1);
